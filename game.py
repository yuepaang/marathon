...
class Game:
    ...
    def _load_map(
        self, map_data: Dict
    ) -> Tuple[Dict[Tuple[int, int], str], Dict, Dict]:
        map_template = {}
    ...
    def _handle_agent_collision_different_team(
        self, collision_type: str, attacker: Agent, defender: Agent
    ):
        ...
        # 回到起始地点
        self.logs.append(
            f"player[{attacker.player_id}]的agent[{attacker.id}]抓获player[{defender.player_id}]的agent[{defender.id}],抢夺了{score_delta}金币"
        )
    ...
    def _handle_agent_collision_same_team(
        self, collision_type: str, agent_a: Agent, agent_b: Agent
    ):
    ...
    def apply_actions(
        ...
        # 用动作更新next_pos位置
        for agents, actions in [
            (attacker_agents, attacker_actions),
                ...
                for dy in range(-vision_range, vision_range + 1):
                    ...
                    if cell is not None:
                        if cell["type"] == CellType.COIN:
                            ...
                            view["powerups"].append(
                                {"x": vx, "y": vy, "powerup": str(cell["powerup"])}
                            )
                    ...
                    for s in ss:
                        if s["id"] == agent.id:
                            ...
                            if (
                                agent.role == agent.ATTACKER
                                and "invisibility" in s["powerups"]
                            ):
                                continue
        ...
        for pos, cell in self.map.items():
            if cell["type"] == CellType.WALL:
                ...
                map_state["powerups"].append(
                    {"x": pos[0], "y": pos[1], "powerup": str(cell["powerup"])}
                )
